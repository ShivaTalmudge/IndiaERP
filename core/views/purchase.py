...
def purchase_create(request):
    ...
    suppliers = Supplier.objects.filter(company=request.company)
...
def purchase_detail(request, pk):
    order = get_object_or_404(PurchaseOrder, company=request.company, pk=pk)
...
def purchase_cancel(request, pk):
    ...
    order = get_object_or_404(PurchaseOrder, company=request.company, pk=pk, status="received")
